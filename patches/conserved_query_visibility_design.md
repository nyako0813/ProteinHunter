...
**訂正(2026-09-20)**: 下の「背景・問題」が根拠にしている Tier A の実例(HdrD1-Mer、
MtpA-MtpC、Nif*)が Negative_hit に落ちるという観察は、陰性参照ゲノムの取り違え
(`negative/Sulfolobus_solfataricus` に陽性ゲノムのコピーが入っていた)に大きく
起因していたことが後で判明した。修正後は17パートナーのうち14が Candidates に入る。
警告の仕組み自体は、本当に広く保存されたクエリ(例: DnaK は strong ヒット)には
依然有効だが、実装した警告文は上記の Tier A 例を根拠として挙げない表現に改めた。
以下の設計本文は当時の記録として未変更。
