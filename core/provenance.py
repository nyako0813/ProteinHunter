...
* a sidecar YAML next to the Excel workbook (``<stem>.run_provenance.yaml``)
  holding the full effective ``Config``, so a temporary, uncommitted edit of
...
def _config_hash_inputs(config: Any, config_path: Path | None) -> list[tuple[str, Path | None]]:
    """The files whose content defines scoring behavior, labelled.
    ...
    domain-family map. Data files (STRING/GEO caches, the PIH bundle, BLAST
    databases) are deliberately not hashed: they are data, not configuration,
    and reading them would cost real run time. Reference genomes are checked
    separately by core/reference_genomes.py.
    """
...
def compute_config_hash(inputs: list[tuple[str, Path | None]]) -> str:
    """SHA-256 (hex) over the labelled contents of ``inputs``.

    A ``None`` path (the built-in default is in use), a missing file and an
