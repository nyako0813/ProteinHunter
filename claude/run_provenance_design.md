...
Status: **実装完了・実データ検証済み・PR #21としてpush済み(未マージ)。**
PR #16・#17・#19・#20は全てmainにマージ済み(#18はクローズ済み)。
全609テストパス。設計からの差分は末尾「実装結果(2026-09-20、Claude Code
実装)」参照。
...
`geo_coexpression_enabled`, `pih_evidence_bundle`, `consider_cross_species_matches`,
`ranking_metric`, 各種cap/weight...)。git commit単位でコードは追跡できるが、
...
- 実際に過去起きた事故: Phase 6-8 Stage 1の実データ検証で「分離幅
  +32.89」という誤った数値が一度報告され、後で「STRING/GEOを無効化した
...
2. **サイドカーファイル(常時出力)**: 出力Excel/Wordと同じディレクトリに、
   実行に使われた**実効設定の全体**(`Config` dataclass全体、CLI引数や
...
- `git_commit`/`git_dirty` の取得は `subprocess.run(["git", "rev-parse",
  "--short=7", "HEAD"], cwd=repo_root, ...)` と
  `git status --porcelain` を使う(新規の外部依存を増やさない —
...
- **configハッシュの対象**: `config.yaml` 本体に加え、スコアリング挙動を
  左右する2つの外部YAML —
  `scoring_engine_config`(`config/scoring_engine.example.yaml` 相当)と
  `functional_complementarity_ruleset`
  (`config/functional_complementarity_rules.v1.yaml`) — も
  **実際に使われたパスのファイル内容**を連結してSHA-256を取る。
...
- 実行の最後に `write_provenance_sidecar(provenance, config,
  config.paths.output_excel)` を呼ぶ。Word/Excelどちらかしか有効でない
...
`01_Index` シートの `INDEX_ROWS_V2` テーブルの**上**、既存の
...
- `tests/test_provenance.py`(新規): git無し環境(一時ディレクトリを
  `cwd` にしてgitリポジトリでない状態を作る)で `collect_run_provenance`
  が例外を出さず `git_commit=None` を返すこと。configハッシュが
...
negative/Sulfolobus_solfataricus/`に実際には*Methanococcus maripaludis*
...
留める。** 上記「実装内容」1〜4節に、この統合方法を反映済み。

## 実装結果(2026-09-20、Claude Code実装・PR #21)

PR #16・#17・#19・#20の全マージ確認後に着手。実装・テスト完了、
全609テストパス。実データ検証(`config.yaml`のコピーで
`max_candidates_per_query`を200→150に変更し2回実行)で、config
フィンガープリントが`ae95eec8`→`8b48b237`に変化し、サイドカーYAML
の中身がそれぞれ200/150になっていること、Excel `01_Index`とWord
タイトルページ双方にfingerprintブロックが出ることを確認済み。両実行とも
`reference_genome_check_passed=True`。

設計からの差分(いずれも妥当な追加判断と評価):

- `collect_run_provenance`にキーワード引数で`config_path`等を追加。
  `Config`データクラス自体は読み込み元ファイルのパスを保持していない
  ため、呼び出し側から明示的に渡す必要があった。
- configハッシュの対象に、設計書記載の2ファイルに加えて
  `domain_family_map_path`も追加。スコアリング挙動(ドメイン相補性
  評価)を左右する外部ファイルという点で`functional_complementarity_
  ruleset`と同種のため、対象漏れだったと判断し追加。
- ハッシュ計算前に改行をLFへ正規化。WSL/Windowsで同一内容のファイルが
  改行コードの違いだけで別ハッシュになる(=別config扱いになる)事故を
  防ぐため。設計書は単一マシンでの利用を暗黙に想定していたが、
  このプロジェクトは実際に複数マシンをまたいで運用されているため、
  必要な補正。
- git判定: `repo_root`がgitのトップレベルと一致しない場合は
  `git_commit`/`git_dirty`を`None`にする。配布zipを別のgitリポジトリ
  (例: このリポジトリ自体のクローンの中)に展開した場合に、無関係な
  親リポジトリのSHAを誤って拾わないようにするための安全策。

## 実装順序に関する注意(2026-09-20追記、解消済み)

実装着手条件(PR #16・#17・#19・#20が全てmainにマージされていること、
`core/reference_genomes.py`がmainに入っていること)は、着手前に
Claude Code側で確認済み。想定していたファイル競合(`output/word_report.py`
のタイトルページ部分と、[[conserved_query_visibility_design]]が触る
5.7節は別セクション)も、実際の実装で問題にならなかった。