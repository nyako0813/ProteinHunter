...
INTERACTION_SCORE_EXPLANATIONS: tuple[tuple[str, str], ...] = (
    ...
    (
        ...
        "ceiling of 18) and 'interaction' (cap 70, interaction_score/100) -- plus an "
        "independently-applied negative_hit_strength penalty (a second, separate read of the "
        "same record property already folded into interaction_priority_score elsewhere, not a "
        "double subtraction of that value). Both caps are PROVISIONAL, see "
        "claude/final_score_integration_investigation.md for the real-data check behind the "
        "30/70 split. When interaction_score is unavailable for a pair, Final Score falls back "
